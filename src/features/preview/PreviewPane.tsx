import { Fragment, createElement, lazy, Suspense, useEffect, useMemo, useState, type ComponentPropsWithoutRef, type ComponentType, type ReactNode } from "react";
...
const rehypePlugins = [rehypeSlug, rehypeKatex, rehypeTaskOffsets];
  ...
  return {
    ...
    code: ({ className, children, ...props }: ComponentPropsWithoutRef<"code">) => {
      if (/language-mermaid/.test(className || "")) {
        return (
          <Suspense fallback={<p className="text-sm text-muted">{labels.loadingMermaid}</p>}>
...
export function PreviewPane({
  ...
  }, [parsed.body, shouldCompileMdx, t]);
  ...
  return (
    <section
      ...
      onScroll={onScroll}
    >
      <article ref={articleRef} className="memoir-preview prose prose-neutral px-9 pb-24 pt-10 dark:prose-invert">
        {error ? (
