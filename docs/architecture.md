...
Application and component styles live in colocated `stylex.create` definitions. Shared colors, typography, motion, layout values, themes, and media queries come from `src/styles/tokens.stylex.ts`; document theme classes and runtime values are applied through `src/styles/document-theme.ts`.

The CSS entry point intentionally contains only the document reset, KaTeX vendor CSS, and rules for markup generated by Markdown, MDX, rehype, highlight.js, and Mermaid. Generated-markup rules must stay scoped below `[data-preview-root]`. Use stable `data-*` attributes for behavior and tests instead of generated StyleX class names. Run `bun run style:check` to enforce these boundaries.
